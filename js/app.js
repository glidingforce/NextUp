...
const TR={
  en:{
    ...
    editExercise:'Edit Exercise',save:'Save',cancel:'Cancel',previous:'Previous',
  ...
  he:{
    ...
    editExercise:'עריכת תרגיל',save:'שמור',cancel:'ביטול',previous:'קודם',
...
function signInWithGoogle(usePopup){
  ...
  if(usePopup){
    window.open('https://accounts.google.com/o/oauth2/v2/auth?'+p,'gauth','width=500,height=600,left=100,top=100');
  } else {
...
async function handleOAuthCallback(){
  ...
  if(window.opener&&!window.opener.closed){
    try{window.opener.postMessage({type:'nu_oauth_token',token},'*');}catch(e){}
    window.close();
    return true;
  }
...
let _pendingSyncAction=null;
function _setupPopupListener(){
  window.addEventListener('message',async(ev)=>{
    if(!ev.data||ev.data.type!=='nu_oauth_token')return;
    const token=ev.data.token;if(!token)return;
    storeToken(token);
    if(!currentUser){
      try{
        const r=await fetch('https://www.googleapis.com/oauth2/v3/userinfo?access_token='+token);
      ...
      }catch(e){console.warn('popup userinfo',e);}
    } else {
...
async function _ensureToken(){
  let tok=getToken();
  if(tok)return tok;
  // Token expired: open popup so the page never reloads and local data is safe
  return new Promise(resolve=>{
    ...
    const handler=(ev)=>{
      ...
      clearTimeout(tid);
      const newTok=ev.data.token;
      if(newTok){storeToken(newTok);_setSyncStatus('ok');resolve(newTok);}
      else{_setSyncStatus('error');resolve(null);}
    };
...
// Lightweight per-exercise weight log: keeps last 3 entries only, tiny footprint
function getWeightLog(wk,id){
  try{const raw=localStorage.getItem(sk('wlog_'+wk+'_'+id));return raw?JSON.parse(raw):[];}
  catch(e){return[];}
}
function pushWeightLog(wk,id,weight){
  if(!weight||weight<=0)return;
  const log=getWeightLog(wk,id);
  log.unshift({w:weight,d:Date.now()});
  if(log.length>3)log.length=3;
  localStorage.setItem(sk('wlog_'+wk+'_'+id),JSON.stringify(log));
}
...
function renderEdit(){
  ...
  const items=w.exercises.map((ex,ei)=>{const img=getImg(w.key,ex.id);return'<div class="exercise-item">'+(img?'<div class="ex-thumb"><img src="'+img+'" alt=""></div>':'<div class="ex-thumb">🏋️</div>')+'<div class="ex-info"><div class="ex-name">'+esc(ex.name)+'</div><div class="ex-meta">'+ex.sets+' × '+esc(ex.reps)+(ex.weight>0?' · '+ex.weight+t('weightUnit'):'')+'</div></div><div class="ex-actions"><button class="btn-icon" data-a="moveExUp" data-wi="'+wi+'" data-ei="'+ei+'" '+(ei===0?'disabled style="opacity:.3"':'')+'>▲</button><button class="btn-icon" data-a="moveExDown" data-wi="'+wi+'" data-ei="'+ei+'" '+(ei===n-1?'disabled style="opacity:.3"':'')+'>▼</button><button class="btn-icon" data-a="editEx" data-wi="'+wi+'" data-ei="'+ei+'">✏️</button><button class="btn-icon" data-a="delEx" data-wi="'+wi+'" data-ei="'+ei+'">🗑️</button></div></div>';}).join('');
  $app().innerHTML=
...
function renderWorkout(){
  ...
  const isImperial=getUnits()==='imperial';
  // Weight display: editable input on last set, static text otherwise
  let weightBox;
  if(isLastSet&&ex.weight>0){
    const dispVal=isImperial?kgToSmartLbs(ex.weight):ex.weight;
    const unitLbl=isImperial?'lbs':'kg';
    weightBox='<div class="stat-box"><div class="stat-label">'+t('weight')+'</div><input id="last-set-weight" class="stat-value stat-input" type="number" step="0.5" inputmode="decimal" value="'+dispVal+'" style="width:64px;background:transparent;border:none;border-bottom:1px solid var(--accent,#d8ff3e);text-align:center;color:inherit;font:inherit;"><span style="font-size:11px;opacity:.6;margin-left:3px;">'+unitLbl+'</span></div>';
  } else {
    const wDisp=ex.weight>0?(isImperial?kgToSmartLbs(ex.weight)+' lbs':ex.weight+' kg'):'BW';
    weightBox='<div class="stat-box"><div class="stat-label">'+t('weight')+'</div><div class="stat-value">'+wDisp+'</div></div>';
  }
  // Tiny previous-weights hint (last up to 3 entries), single compact line
  let prevHint='';
  if(isLastSet){
    const log=getWeightLog(w.key,ex.id);
    if(log.length){
      const vals=log.map(e=>isImperial?kgToSmartLbs(e.w):e.w).join(' · ');
      prevHint='<div style="font-size:11px;opacity:.5;text-align:center;margin-top:2px;">'+t('previous')+': '+vals+(isImperial?' lbs':' kg')+'</div>';
    }
  }
  $app().innerHTML='<div class="workout-wrap"><div class="progress-row"><span class="progress-lbl">'+t('exercise')+' '+(ei+1)+' / '+w.exercises.length+'</span><span class="exit-lnk" data-a="exitW">⏹ '+t('exit')+'</span></div><div class="progress-track"><div class="progress-fill" style="width:'+pct+'%"></div></div>'+(img?'<img class="ex-image" src="'+img+'" alt="'+esc(ex.name)+'">':'')+'<div class="ex-name-large">'+esc(ex.name)+'</div><div class="ex-group-lbl">'+esc(w.icon)+' '+esc(w.name)+'</div><div class="set-display"><div class="set-label">'+t('set')+'</div><div><span class="set-num">'+setNum+'</span><span class="set-total"> / '+ex.sets+'</span></div></div><div class="stats-grid"><div class="stat-box"><div class="stat-label">'+t('reps')+'</div><div class="stat-value">'+esc(ex.reps)+'</div></div>'+weightBox+'</div>'+prevHint+(ex.notes?'<div class="notes-box">💡 '+esc(ex.notes)+'</div>':'')+'<div class="spacer"></div><button class="btn btn-primary" data-a="doSet">'+btnLabel+'</button><button class="btn btn-skip" data-a="skipEx">'+t('skipExercise')+'</button></div>';
}
...
function doSet(){
  ...
  // On the last set, read the (possibly edited) weight input before proceeding
  if(lastSet){
    const inp=document.getElementById('last-set-weight');
    if(inp){
      const raw=parseFloat(inp.value);
      if(!isNaN(raw)&&raw>0){
        const kgVal=getUnits()==='imperial'?lbsToNearestKg(raw):Math.round(raw*2)/2;
        if(kgVal!==ex.weight){
          pushWeightLog(w.key,ex.id,ex.weight); // log the weight just finished, before overwriting
          ex.weight=kgVal;
          saveWorkouts();
        }
      }
...
document.addEventListener('click',function(e){
  ...
  if(a==='moveExUp'){const wi3=+T.dataset.wi,ei3=+T.dataset.ei;if(ei3>0){const arr=S.workouts[wi3].exercises;[arr[ei3-1],arr[ei3]]=[arr[ei3],arr[ei3-1]];saveWorkouts();render();}return;}
  if(a==='moveExDown'){const wi3=+T.dataset.wi,ei3=+T.dataset.ei;const arr=S.workouts[wi3].exercises;if(ei3<arr.length-1){[arr[ei3+1],arr[ei3]]=[arr[ei3],arr[ei3+1]];saveWorkouts();render();}return;}
  if(a==='saveEx'){
...
async function initApp(){
  ...
  // 1. Set up popup listener (for token refresh without page reload)
  _setupPopupListener();
  // 2. Handle OAuth callback (redirect flow - first sign-in)
